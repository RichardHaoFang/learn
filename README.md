...
>在JVM的支持下java支持动态绑定，而c++则没有这种机制。在java中无法通过实例访问
>父类被覆盖的方法,但是可以在实例内部通过super访问父类被覆盖的方法。这一点java
>更符合面向对象的设计,这也是java设计的初衷。虽然C++这一点更具有灵活性，但是大多
>数时候，覆盖父类意味着子类想做些特殊的处理。如果能够跳过子类的特殊处理，就会打
>开一个无法控制的缺口，会导致很多很多问题。唯一可以调用父类方法的地方，就只有类
>内部。子类自己控制，什么时候该调用父类，什么时候做自己的处理。  
...
>java和c++都可以重载父类同名方法/函数，但是c++不重载父类参数名个数相的函数，原因
>是这种情况会令人迷惑不解，易使程序员犯错，而java在jdk1.5之后引入了@Override注解，
>解决了这个问题。此外c++可以通过引入作用域的方法实现这种重载。
>对于成员变量，c++和java都不能实现多态，java对同名变量引用的确定在编译期完成。对
>于向上转型，java实例可以访问父类具有访问权限的变量。  
>java和c++静态方法都不支持多态。
>### 字符串
>在java中"\"转义普通字符会报错（正则会对字符的转义报异常，而不会对非字母字符报异
>常，因为java开发者认为转义字母在正则将来的扩展中可能会有新的含义），C/C++中转义
>普通字符会忽略转义。
>js中转义普通祖父会忽略转义（忽略转义=去掉转义符"\"）(js正则也一样，c/c++未实验)。
## CH4 访问权限
>### 包管理
